...
| # | 説明 | レベル | ロール |
| :-----------: | :------------------------------------------------------------------------------- | :---: | :--: |
| **2.3.1** | **検証:** 許可される文字セットは、ビジネス要件に準拠し続けるように、定期的にレビューおよび更新されている。 | 2 | D/V |
...
| **9.5.4** | **検証:** プロトコル実装は、デシリアライゼーション、インジェクション、リクエストスマグリング、ステートマシンエッジケースを対象としたファジングおよび静的解析を受けている。 | 3 | V |
| **9.7.4** | **検証:** システムが意図の不一致またはポリシー/制約違反を検出した場合、インシデントを記録し、適切なチームに警告し、インシデントを使用して防御を強化 (テストの追加/更新、ポリシーの厳格化、検出ルールの更新) して、同じ障害が再び発生する可能性を減らしている。 | 3 | D/V |
| **9.8.3** | **検証:** CI/CD は、境界付き動作と主要な安全性の不変条件 (例: クロスドメインメッセージングなし、無制限のファンアウトなし) を証明するための、マルチエージェントプロトコルのプロパティベースのテストまたは同等の解析を含んでいる。 | 3 | V |
