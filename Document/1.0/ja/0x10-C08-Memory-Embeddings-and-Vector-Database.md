...
| **8.1.4** | **検証:** 取り込まれたすべてのドキュメントは書き込み時に、ソース、書き込み者アイデンティティ (認証済みユーザーまたはシステムプリンシパル)、タイムスタンプ、バッチ ID、エンベディングモデルのバージョンでタグ付けされている。 | 2 |
...
| **8.3.2** | **検証:** システムの定義された機能に必要な情報のみがメモリに保持されている (ユーザー設定や会話の決定などであり、クレデンシャルや会話の全文は保持されない)。 | 1 |
...
| **8.3.6** | **検証:** メモリは、保持削除とは別個独立した操作を通じて、セキュリティ上の理由 (隔離、選択的消去、完全リセット) でリセットできる。 | 2 |
| **8.3.8** | **検証:** 隔離されたコンテンツは調査のために保持されているが、隔離されている間は検索結果から除外されている。 | 2 |
