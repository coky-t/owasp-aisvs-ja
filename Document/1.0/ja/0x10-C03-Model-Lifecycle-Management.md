...
| **3.1.3** | **検証:** モデルアーティファクトの署名と完全性チェックサムは、デプロイメント承認時とロード時に検証され、署名されていないアーティファクト、改竄されたアーティファクト、一致しないアーティファクトは拒否されている。 | 2 |
...
| **3.2.6** | **検証:** すべてのモデル変更 (デプロイメント、構成、廃止) は、タイムスタンプ、認証されたアクターのアイデンティティ、変更タイプ、前後のステータス、トレースメタデータ (環境と消費サービス/エージェント)、モデル識別子 (バージョン/ダイジェスト/署名) を含む不変の監査レコードを生成している。 | 2 |
