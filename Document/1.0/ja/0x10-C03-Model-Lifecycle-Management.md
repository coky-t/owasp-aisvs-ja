...
| # | 説明 | レベル |
| :--------: | ------------------------------------------------------------------------------------------------------------------- | :---: |
| **3.1.1** | **検証:** モデルレジストリはデプロイされたすべてのモデルアーティファクトのインベントリを維持している。 | 1 |
| **3.1.2** | **検証:** すべてのモデルアーティファクト (重み、構成、トークナイザ、ベースモデル、ファインチューン、アダプタ、安全性/ポリシーモデル) は認可されたエンティティで暗号署名されている。 | 2 |
| **3.1.3** | **検証:** モデルアーティファクトの署名と完全性チェックサムは、デプロイメント承認時とロード時に検証されている。署名されていないアーティファクト、改竄されたアーティファクト、一致しないアーティファクトは拒否されている。 | 2 |
| **3.1.4** | **検証:** リネージと依存関係の追跡は、環境 (開発、ステージング、本番など) ごとにすべての消費サービスとエージェントを識別する、依存関係グラフを維持している。 | 3 |
...
| **3.2.5** | **検証:** セキュリティテストは、バージョン管理された評価ハーネスを使用して、エージェントワークフロー、ツールと MCP の統合、RAG とメモリのインタラクション、マルチモーダル入力、ガードレール (安全性モデルまたは検出サービス) をカバーしている。At minimum, tests demonstrate rejection of prompt injection probes from a maintained corpus and correct sanitization of tool outputs before model context inclusion. | 2 |
| **3.2.6** | **検証:** すべてのモデル変更 (デプロイメント、構成、廃止) は不変の監査レコードを生成している。各レコードにはタイムスタンプ、認証されたアクターのアイデンティティ、変更タイプ、前後のステータス、トレースメタデータ (環境と消費サービス/エージェント)、モデル識別子 (バージョン/ダイジェスト/署名) を含んでいる。 | 2 |
