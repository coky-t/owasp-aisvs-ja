...
| **4.8.5** | **検証:** モバイルまたはエッジ推論アプリケーションは、改変されたバイナリ、再パッケージされたアプリ、アタッチされた計装フレームワークを検出してブロックする、プラットフォームレベルの改竄防止保護 (コード署名、検証済みブート、ランタイム自己完全性チェックなど) を実装している。 | 3 | D/V |
| **4.8.6** | **検証:** エッジデバイスまたはモバイルデバイスにデプロイされたモデルはパッケージ化時に暗号署名され、デバイス上のランタイムがロードまたは推論する前にこれらの署名またはチェックサムを検証している。検証されていないモデルや改変されたモデルは拒否される必要がある。 | 3 | D/V |
