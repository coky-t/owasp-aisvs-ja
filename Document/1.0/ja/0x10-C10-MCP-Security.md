...
| **10.3.4** | **検証:** MCP サーバーは、DNS 再バインディング攻撃を防ぐために、すべての HTTP ベースのトランスポート (SSE およびストリーミング可能な HTTP を含む) の `Origin` ヘッダと `Host` ヘッダの両方を個別に検証している。いずれかのヘッダが欠落している、一致しない、または信頼できないオリジンからのリクエストは拒否されている。 | 2 |
